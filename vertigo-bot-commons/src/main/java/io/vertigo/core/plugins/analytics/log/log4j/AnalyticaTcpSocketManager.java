 ...
 */
public class AnalyticaTcpSocketManager extends AbstractSocketManager {
	/**
	...
	private static final TcpSocketManagerFactory<AnalyticaTcpSocketManager, FactoryData> FACTORY = new TcpSocketManagerFactory<>();
	 ...
	 */
	@Deprecated
	public AnalyticaTcpSocketManager(final String name, final OutputStream os, final Socket socket,
	 ...
	 */
	@Deprecated
	public static AnalyticaTcpSocketManager getSocketManager(final String host, final int port, final int connectTimeoutMillis,
	 ...
	 * @return A TcpSocketManager.
	 */
	public static AnalyticaTcpSocketManager getSocketManager(final String host, int port, final int connectTimeoutMillis,
		...
		return (AnalyticaTcpSocketManager) getManager("TCP:" + host + ':' + port, new FactoryData(host, port,
	 ...
	 */
	protected static class TcpSocketManagerFactory<M extends AnalyticaTcpSocketManager, T extends FactoryData>
		...
		M createManager(final String name, final OutputStream os, final Socket socket, final InetAddress inetAddress, final T data) {
			return (M) new AnalyticaTcpSocketManager(name, os, socket, inetAddress, data.host, data.port,
		...
		Socket createSocket(final T data) throws IOException {
			...
			for (final InetSocketAddress socketAddress : socketAddresses) {
				try {
					return AnalyticaTcpSocketManager.createSocket(socketAddress, data.socketOptions, data.connectTimeoutMillis);
				} catch (final IOException ex) {
					ioe = ex;
				}
			}
