package io.vertigo.ai.plugins.nlu.rasa.helper;
...
public class RasaHttpSenderHelper {

	private RasaHttpSenderHelper() {
		// helper
	}
	...
	public static String launchTraining(final String rasaUrl, final Map<String, Object> map) {
		...
		final byte[] output = FileIOHelper.getYamlByteArrayFromMap(map);
	...
	public static void putModel(final String rasaUrl, final String filename) {
		...
		final ObjectMapper mapper = FileIOHelper.createCustomObjectMapper();
		final ObjectNode node = FileIOHelper.createNode(mapper, "model_file", "models/" + filename);
		final String json = FileIOHelper.getJsonStringFromObject(mapper, node);
		...
		sendRequest(client, request, BodyHandlers.ofString(), 204);

	}

	public static RasaParsingResponse getIntentFromRasa(final String rasaUrl, final MessageToRecognize message) {
		final ObjectMapper mapper = FileIOHelper.createCustomObjectMapper();
		final String json = FileIOHelper.getJsonStringFromObject(mapper, message);
		...
		return FileIOHelper.getObjectFromJson(mapper, response.body(), RasaParsingResponse.class);
	}
