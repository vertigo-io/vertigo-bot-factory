...
public final class InfluxDbTimeSeriesPlugin implements TimeSeriesPlugin, Activeable {
	...
	@Inject
	public InfluxDbTimeSeriesPlugin(@ParamValue("host") final String host, @ParamValue("user") final String user,
			@ParamValue("password") final String password, @ParamValue("dbNames") final Optional<String> dbNamesOpt) {
	...
	public void createDatabases() {
		final Set<String> existingDatabases = influxDB.query(new Query("SHOW DATABASES", null)).getResults().get(0)
				.getSeries().get(0).getValues().stream().map(values -> (String) values.get(0))
	...
	private TimedDatas executeFlatTimedTabularQuery(final String appName, final String queryString) {
		...
		if (series != null && !series.isEmpty()) {
			...
			final List<TimedDataSerie> dataSeries = serie.getValues().stream().map(value -> {
				final Map<String, Object> mapValues = buildMapValue(columns, value);

				return new TimedDataSerie(LocalDateTime
						.parse(value.get(0).toString(), DateTimeFormatter.ISO_OFFSET_DATE_TIME)
						.toInstant(ZoneOffset.UTC), mapValues);
	...
	private TimedDatas executeTimedTabularQuery(final String appName, final String queryString) {
		...
		if (series != null && !series.isEmpty()) {
			// all columns are the measures
			final List<String> seriesName = new ArrayList<>();
			seriesName.addAll(series.get(0).getColumns().subList(1, series.get(0).getColumns().size()));// we remove the
			// first one
			seriesName.addAll(series.get(0).getTags().keySet());// + all the tags names (the group by)

			final List<TimedDataSerie> dataSeries = series.stream().map(mySeries -> {
				...
				return new TimedDataSerie(LocalDateTime
						.parse(mySeries.getValues().get(0).get(0).toString(), DateTimeFormatter.ISO_OFFSET_DATE_TIME)
						.toInstant(ZoneOffset.UTC), mapValues);
			}).collect(Collectors.toList());
	...
	private TabularDatas executeTabularQuery(final String appName, final String queryString) {
		...
		if (series != null && !series.isEmpty()) {
			...
			final List<TabularDataSerie> dataSeries = series.stream().map(mySeries -> {
				final Map<String, Object> mapValues = buildMapValue(mySeries.getColumns(), mySeries.getValues().get(0));
	...
	private TimedDatas executeTimedQuery(final String appName, final String q) {
		...
		if (seriesList != null && !seriesList.isEmpty()) {
			...
			final List<TimedDataSerie> dataSeries = series.getValues().stream()
					.map(values -> new TimedDataSerie(
							LocalDateTime.parse(values.get(0).toString(), DateTimeFormatter.ISO_OFFSET_DATE_TIME)
							.toInstant(ZoneOffset.UTC),
							buildMapValue(series.getColumns(), values)))
					.collect(Collectors.toList());
			return new TimedDatas(dataSeries,
					new ArrayList<>(series.getColumns().subList(1, series.getColumns().size())));// we remove the first
			// one
		}
		return new TimedDatas(Collections.emptyList(), Collections.emptyList());
	}
	...
	@Override
	public TimedDatas getClusteredTimeSeries(final String appName, final ClusteredMeasure clusteredMeasure,
			final DataFilter dataFilter, final TimeFilter timeFilter) {
		...
		Assertion.checkState(!clusteredMeasure.getThresholds().isEmpty(),
				"For clustering the measure '{0}' you need to provide at least one threshold",
		...
		final String standardwhereClause = buildWhereClause(dataFilter, timeFilter);// the where clause is almost the
		...
		// for each cluster defined by the thresholds we add a subquery (after benchmark
		...
		for (int i = 0; i <= clusteredMeasure.getThresholds().size(); i++) {
			...
			// we add the where clause of the cluster value > threshold_1 and value <=
			// threshold_2
			appendMeasureThreshold(minThreshold, maxThreshold, fieldName, clusteredMeasure.getMeasure(),
					dataFilter.getMeasurement(), standardwhereClause, timeFilter.getDim(), fromClause, i);

			// we construct the top select clause. we use the max as the aggregate function.
			// No conflict possible
			selectClause.append(" max(\"").append(fieldName).append('_').append(i).append("\") as \"")
			.append(clusterName(minThreshold, maxThreshold, clusteredMeasure.getMeasure())).append('"');
			if (i < clusteredMeasure.getThresholds().size()) {
				selectClause.append(',');
				fromClause.append(", ");
			}
		...
		// the global query
		final String request = new StringBuilder().append("select ").append(selectClause).append(" from ")
				.append(fromClause).append(" where time > ").append(timeFilter.getFrom()).append(" and time <")
				.append(timeFilter.getTo()).append(" group by time(").append(timeFilter.getDim()).append(')')
				.toString();
	...
	@Override
	public TimedDatas getTabularTimedData(final String appName, final List<String> measures,
			final DataFilter dataFilter, final TimeFilter timeFilter, final String... groupBy) {
		final StringBuilder queryBuilder = buildQuery(measures, dataFilter, timeFilter, true);

		final String groupByClause = Stream.of(groupBy).collect(Collectors.joining("\", \"", "\"", "\""));
	...
	@Override
	public TabularDatas getTabularData(final String appName, final List<String> measures, final DataFilter dataFilter,
			final TimeFilter timeFilter, final String... groupBy) {
		final StringBuilder queryBuilder = buildQuery(measures, dataFilter, timeFilter, true);
	...
	public List<String> getTagValues(final String appName, final String measurement, final String tag) {
		final String queryString = new StringBuilder("show tag values on ").append("\"").append(appName).append("\"")
				.append(" from ").append("\"").append(measurement).append("\"").append("  with key= ").append("\"")
		...
		if (seriesList != null && !seriesList.isEmpty()) {
			final Series series = seriesList.get(0);
			return series.getValues().stream().map(values -> values.get(1).toString()) // always the second columns
					.collect(Collectors.toList());
		}
	...
	@Override
	public TimedDatas getTimeSeries(final String appName, final List<String> measures, final DataFilter dataFilter,
			final TimeFilter timeFilter) {
		Assertion.checkNotNull(measures);
		Assertion.checkNotNull(dataFilter);
		Assertion.checkNotNull(timeFilter.getDim());// we check dim is not null because we need it
		// ---
		final String q = buildQuery(measures, dataFilter, timeFilter, true).append(" group by time(")
				.append(timeFilter.getDim()).append(')').toString();
	...
	@Override
	public TabularDatas getTops(final String appName, final String measure, final DataFilter dataFilter,
			final TimeFilter timeFilter, final String groupBy, final int maxRows) {
		final String queryString = new StringBuilder().append("select top(").append("\"top_").append(measure)
				.append("\", \"").append(groupBy).append("\", ").append(maxRows).append(") as \"").append(measure)
				.append('"').append(" from ( select ").append(buildMeasureQuery(measure, "top_" + measure))
				.append(" from ").append(dataFilter.getMeasurement()).append(buildWhereClause(dataFilter, timeFilter))
				.append(" group by \"").append(groupBy).append('"').append(')').toString();
	...
	public void insertMeasure(final String dbName, final Measure measure) {
		...
		influxDB.write(Point.measurement(measure.getMeasurement())
				.time(measure.getInstant().toEpochMilli(), TimeUnit.MILLISECONDS).fields(measure.getFields())
				.tag(measure.getTags()).build());

	}

	@Override
	public void insertMeasures(final String dbName, final List<Measure> measures) {
		// with influxdb we make the choice to always be in batch mode for performance
		// so we can safely iterate.
		measures.forEach(measure -> insertMeasure(dbName, measure));
	}

	private static void appendMeasureThreshold(final Integer previousThreshold, final Integer currentThreshold,
			final String clusteredField, final String clusteredMeasure, final String measurement,
			final String standardwhereClause, final String timeDimension, final StringBuilder fromClauseBuilder,
			final int i) {
		fromClauseBuilder.append("(select ").append(buildMeasureQuery(clusteredMeasure, clusteredField + "_" + i))
		.append(" from ").append(measurement).append(standardwhereClause);
		if (previousThreshold != null) {
			fromClauseBuilder.append(" and \"").append(clusteredField).append('"').append(" > ")
			.append(previousThreshold);
		}
		if (currentThreshold != null) {
			fromClauseBuilder.append(" and \"").append(clusteredField).append('"').append(" <= ")
			.append(currentThreshold);
		}
	...
	private static String buildMeasureQuery(final String measure, final String alias) {
		...
		final StringBuilder measureQueryBuilder = new java.lang.StringBuilder(aggregateFunction.getVal1()).append("(\"")
				.append(measureDetails[0]).append("\"");
		// append parameters
		if (!aggregateFunction.getVal2().isEmpty()) {
			measureQueryBuilder.append(aggregateFunction.getVal2().stream().collect(Collectors.joining(",", ", ", "")));
		}
	...
	private static StringBuilder buildQuery(final List<String> measures, final DataFilter dataFilter,
			final TimeFilter timeFilter, final boolean needAggregatedMeasures) {
		Assertion.checkNotNull(measures);
		// ---
		...
		for (final String measure : measures) {
			...
			queryBuilder.append(separator).append(measureQuery);
			separator = " ,";
		}
	...
	private static String buildWhereClause(final DataFilter dataFilter, final TimeFilter timeFilter) {
		final StringBuilder queryBuilder = new StringBuilder().append(" where time > ").append(timeFilter.getFrom())
				.append(" and time <").append(timeFilter.getTo());

		for (final Map.Entry<String, String> filter : dataFilter.getFilters().entrySet()) {
			if (filter.getValue() != null && !"*".equals(filter.getValue())) {
				queryBuilder.append(" and \"").append(filter.getKey()).append("\"='").append(filter.getValue())
				.append('\'');
			}
		}
	...
	private static String clusterName(final Integer minThreshold, final Integer maxThreshold, final String measure) {
	...
	private static Tuple<String, List<String>> parseAggregateFunction(final String aggregateFunction) {
		...
		if (firstSeparatorIndex > -1) {
			return Tuple.of(aggregateFunction.substring(0, firstSeparatorIndex),
