let workspace;
const varTypeDropdownOptions = [];
let pathMediaBlockly;
const varTypeDropdownJiraFieldTypeOptions = [];
...
varTypeDropdownJiraFieldTypeOptions.push(["%{BKY_JIRA_FIELD_TYPE_ATTACHMENT}", "attachment"])
...
// *********************
        ...
        injectionBlockly: function (mode = false) {
            ...
            if (document.contains(document.getElementById(blocklyDivName))) {
                ...
                if (VertigoUi.vueData.scriptIntention.script != null) {
                    parsed = VUiExtensions.methods.fromCode();
                }
                workspace.addChangeListener(VUiExtensions.methods.toCode);
                workspace.addChangeListener(VUiExtensions.methods.importBlocklyTemplate)
                workspace.addChangeListener(VUiExtensions.methods.nameVarValidator)
                return parsed
            }
        }
    }
    ...
    VUiExtensions.methods = {
        ...VUiExtensions.methods,
        nameVarValidator: function (event) {
        ...
        importBlocklyTemplate: function (event) {
            ...
            switch (templateBlock.type) {
                ...
                case 'cb_template_jira':
                    ...
                    fieldJira.push(['attachment', (locale === 'fr_FR' ? "Quel pièce jointe souhaitez vous ajouter ?" : "What is the file you want to attach"), "attachment"])
                    var fieldJiraBlocks = []
